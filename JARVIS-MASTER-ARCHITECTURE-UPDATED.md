...
**Updated:** August 30, 2026 (wake-word/mic-gain recalibration, then a real end-of-turn VAD fix, both added later same day)  
**Status:** Phase 0, Phase 1, and Phase 1.5 are now verified real via `verify-jarvis.ps1` run live on Gavin's actual PC (2026-08-30) — see that script and its `setup-logs/` output, not just this doc, for the current pass/fail state. Phase 3 is code-complete and code-verified as of 2026-08-27 but not yet run live. Phase 2's TTS/STT/wake-word/reply path was already real; as of 2026-08-30 mic capture, audio playback, and end-of-turn detection are also real and code-complete (`bun run dev listen`). First real live run found wake-word sensitivity badly miscalibrated for Gavin's actual mic/room (real scores 0.0000-0.0175 vs. a 0.15 threshold) — a mic-gain fix + lowered threshold went in same-day. Re-run: wake word fired correctly (91% confidence), but the turn then never ended (a fixed VAD threshold couldn't survive the same gain change) — fixed with a self-calibrating per-turn threshold + a soft-clip fix for real clipping the gain also introduced. **Not yet re-run live a third time** — still unconfirmed whether a full round trip actually completes. Full-duplex/interruption remains deliberately unbuilt. See Part 10 below and the ground-truth bullets above for details.  
