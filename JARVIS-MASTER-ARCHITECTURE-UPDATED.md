...
- ⚠️ **Phase 2 (Voice) — TTS/STT real, wake word + mic capture still not.** As of 2026-08-26: `speech-synthesizer.ts` now runs the real Piper binary (local, $0, no API key) and `speech-recognizer.ts` runs real `faster-whisper` via `scripts/whisper_transcribe.py` — both proven with a live TTS→STT round trip (`bun test src/tests/speech-synthesizer.test.ts src/tests/speech-recognizer.test.ts`, correctly transcribed "the quick brown fox..." back from synthesized audio). Run `scripts/setup-voice.sh` first (downloads the Piper binary/model + builds the whisper venv; gitignored, not committed). Streaming in both classes is honestly labeled non-incremental (whole-buffer, not true low-latency streaming). Still NOT real: `wake-word-detector.ts` (still `Math.random()`), and there is no actual microphone capture anywhere — both need real hardware I/O that can't be built or verified from this Linux sandbox; that work has to happen on Gavin's PC. No CLI command reaches any of Phase 2 yet.
