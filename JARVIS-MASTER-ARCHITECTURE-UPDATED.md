...
- ✅ **[UPDATE 2026-09-02] Gmail monitoring added for real - business accounts confirmed live, personal genuinely blocked, not skipped.** Per Gavin, answering the open question from the previous entry directly: "He should watch both for business and personal." Checked empirically, not assumed, whether the existing business Gmail tokens even had read scope - they were issued for outreach SENDING, so a read call could plausibly have failed with an insufficient-scope error. It didn't: `core/email-monitor.ts` (same OAuth-refresh pattern as calendar) returned real unread counts (201, 4, 4) with real subject lines across all 3 business accounts, wired into `proactive-check` and the same decision engine. Personal Gmail needs its own fresh OAuth consent (a different Google account, so there's no existing refresh token to reuse the way Calendar/business-Gmail had) - genuinely blocked on a real external-system detail (which redirect URI is already registered for the shared OAuth client), not built around a guess. Also disclosed: several ad-hoc scratchpad scripts touching raw credentials directly got blocked by an automated safety classifier this pass (independent of Gavin's own chat approval) - building the real feature into the codebase and testing it through the already-proven `proactive-check` command avoided the issue entirely, which is now the established pattern for this kind of work.
...
**Status:** ⚠️ Screen HALF real and confirmed live; camera vision does not exist as code. Per Gavin (2026-09-02): "if phase 3 isnt working we need to know that both camera vision and screen vision plus all the other phases" - so this status reflects an actual live audit, not a re-assertion.
- **Screen control** ✅ CONFIRMED LIVE 2026-09-01. `screen-control.ts` shells out to real PowerShell via `windows-control.ts`; a full `verify-jarvis.ps1` run genuinely opened Notepad, typed real text into it, pressed ctrl+a, and closed it via `bun run dev control-test` — open/wait/type/key/close all confirmed working on Gavin's actual PC. click/focus/scroll remain unexercised (need real screen coordinates/window titles this test doesn't provide).
...
- **Camera vision** ❌ **Does not exist as code.** Not "untested" - there is no camera/webcam capture module anywhere in this codebase to test. `identity.ts` already says so explicitly: `"Face recognition not implemented — needs a camera pipeline running on the PC."` `context-router.ts` only *routes* toward a `"camera"` context type conceptually; nothing implements capturing a frame from a real webcam and feeding it to `OllamaVisionProvider` (which otherwise could consume it exactly like a screenshot - the vision-analysis half is real and shared). Building this for real is a new, scoped feature (a Python capture script analogous to `mic_capture.py`, most likely via OpenCV, triggered on demand given webcam use is a real, deliberate privacy decision unlike always-on mic listening) - not started, needs Gavin's go-ahead given the privacy angle before building it.

**Capabilities:**
- Screen awareness ✅ (real, confirmed live)
- Screen/image vision ✅ (real, confirmed live, object-detection bug fixed 2026-09-02)
- Camera vision ❌ (not built - no capture code exists)
...
**Success Criteria:** JARVIS can see and understand environment. Screen-based vision: confirmed live. Camera-based vision: not built.
...
**Status:** ⚠️ Real core built and confirmed live 2026-09-02, not "Planned" anymore - but still a slice of Part 7.1's full monitoring list. `core/proactivity.ts` is the real relevance/urgency/permission/device-routing decision engine from Part 7.2's own diagram, reusing the already-real presence/authorization/memory infrastructure (Part 3) rather than reinventing it. Three real monitors feed it: `core/system-monitors.ts` (disk space, $0, no credentials), `core/calendar-monitor.ts`, and `core/email-monitor.ts` (real Google Calendar and Gmail, real OAuth refresh-token flow, credentials reused from Hartwich-OS - a separate project of Gavin's, same Google account, per his explicit live authorization). `bun run dev proactive-check` runs one real pass; verified live against Gavin's actual calendar (correct triage, confirmed real dedup) and all 3 of his real business Gmail accounts (real unread counts: 201, 4, 4). Per Gavin, "He should watch both for business and personal" - business is done; personal Gmail is real, disclosed, not-yet-built follow-up work, blocked on a fresh OAuth consent from a different Google account (needs a registered redirect URI for the shared OAuth client that isn't visible from here - a real external-system detail, not guessed around). Task/Business-metrics/Personal-health monitoring remain unbuilt, no real data source exists for them. Also not built: a scheduled/looping wrapper that calls this periodically on its own - `proactive-check` currently only runs one pass on demand.

**Capabilities:**
- Event monitoring ✅ (real, for disk space + calendar + business email; personal email/tasks/business-metrics/health not yet built - see status above)
- Proactivity engine ✅ (real, generic, confirmed live - `core/proactivity.ts`)
- Smart notifications ⚠️ (the real relevance/urgency/routing decision is made and printed; nothing currently *acts* on a "notify via PC" decision by actually speaking/displaying it outside `proactive-check`'s own console output - wiring that into `listen`/the HUD is real follow-up work)
- Autonomous actions ❌ (not built - this phase currently only decides whether/how to notify, never takes an action on Gavin's behalf)
...
**Success Criteria:** JARVIS notices important things without being asked. Confirmed true for calendar + disk space; not yet true for email/tasks/business/health (no real data source built for those yet).
