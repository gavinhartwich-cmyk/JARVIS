...
**Adaptive Processing update (2026-09-04):** see `ARCHITECTURE-UPDATE-ADAPTIVE-PROCESSING.md` for the full directive (Intent/Complexity Router, realtime voice layer, streaming, barge-in, action journal/undo, capability registry, and more), verbatim, for reference. Steps 1-3, 7, 8 landed and are merged into this codebase (see the Adaptive Processing bullets appended to the end of the ground-truth list below); steps 4-6 (Gemini Live realtime integration) are an isolated, not-yet-connected prototype; step 9 is correctly deferred pending step 6. Merge note: the router/capability-registry work was built against an older snapshot of this file that predates most of the live voice-pipeline work below (Chatterbox, HUD, Spotify, screen/video/camera vision, proactive intelligence, persistent conversational memory, etc.) — reconciling the two found that `Orchestrator.processConversation()` (this codebase's real, live-tested conversational dispatcher) already covers what the router's TOOL path was built to fix, so it remains the primary path when a real `Orchestrator` is supplied to `VoiceInterface`; the router/capability-registry/episode-cache genuinely add value on `VoiceInterface`'s no-orchestrator fallback path and are wired there instead of duplicating app-control detection.
...
- ✅ **[UPDATE 2026-09-02/09-03] Third and fourth live `listen` rounds on Gavin's real PC (background/auto-run mode, `start-jarvis.ps1`) - six real, live-found voice-pipeline bugs fixed, one genuinely large latency bug fixed, and one still-open Chatterbox length-latency finding disclosed.** Per Gavin's direct live reports, not inferred from logs alone (he explicitly corrected log-based assumptions twice, e.g. "i can tell you right now it the hud didnt open").
  ...
  - **New (09-04): full master-doc alignment audit, per Gavin - "do a deep look through and make sure everything aligns with all the aspects Jarvis should have."** Read Parts 1-3/7/13's stated principles and traced each against what's actually wired into the live code (not just what comment blocks claim). Three real findings; both real gaps (not the already-fixed Spotify one) were then fixed, per Gavin - "Complete both 1 and 2":
    - **Finding 1 (biggest): zero persistent memory for any real conversation.** `storeMemory()`/`retrieveMemories()`/`searchMemories()` (Part 4.3's "Four-Level Memory") were real and working, but traced to exactly one call site anywhere in the codebase - the separate Phase 1 coding-task pipeline. The live voice/conversational path (`processConversation()`, `conversation-intelligence.ts`) never called any of the three - every JARVIS restart meant zero real memory of anything actually said to him, contradicting invariant #5 and the project's own "persistent... memory" identity. **Real fix:** `processConversation()` now stores a real episodic memory for every real action outcome (play/open/click/file op - "signal" by invariant #5's own framing, not noise), plus a new conservative, fail-closed LLM classifier (`classifyMemorableFact()`, same posture as this file's other classifiers) that extracts lasting facts/preferences/plans from plain conversation - both fire-and-forget so neither adds latency to the spoken reply. **Retrieval** wired in for the first time too: new `searchMemoriesByUtterance()` (real keyword search, disclosed as NOT semantic - no embeddings infra in this project - splits the utterance into real keywords past a stopword list, matches ANY of them, ranks by the real `importance` column) called from `conversation-intelligence.ts`'s `assemblePrompt()` before every reply. **Live-verified against the real Postgres DB, not just typechecked:** stored a real test memory ("Gavin has recently been really into jazz music"), confirmed `searchMemoriesByUtterance("what kind of music do I like lately")` genuinely retrieved it (first pass surfaced irrelevant old task-pipeline rows instead - a coincidental keyword collision on "like", fixed by widening the stopword list, then re-verified correct) - test rows deleted afterward, not left in the real table.
    - **Finding 2: conversational actions bypassed authorization/audit - including a false comment claiming otherwise.** `executeSpotifyIntent()`/`executeFileIntent()` ran real actions with ZERO `authorizationEngine.authorize()` call, unlike `executeAppControlIntent()`/`executeClickIntent()` (already gated via `ScreenControl`). Worse: `core/file-manager.ts`'s own header comment claimed it WAS "gated through the same real authorizationEngine... see orchestrator.ts's executeFileIntent()" - false, until this fix. Separately, even where authorization did run (computer-control/click), only the authorize/deny DECISION was ever audit-logged, never the actual outcome - invariant #12 ("every action is auditable") didn't hold past the authorization step. **Real fix:** new shared `authorizeConversationalAction()`/`auditActionOutcome()` helpers in orchestrator.ts, now called by both `executeSpotifyIntent()` and `executeFileIntent()` (same "normal" risk tier `computer_control` already uses); `ScreenControl.executeSequence()` now also audit-logs the real outcome (not just the auth decision) for every open/close/click call, since all three funnel through that one method. `file-manager.ts`'s comment corrected to match reality, with a note on what was wrong and why.
    - **Finding 3 (proactive-vs-reactive sweep): nothing else found.** Searched the rest of the codebase for the same "JARVIS could act but asks the user to instead" pattern as the just-fixed Spotify gap - app-control, click, file ops, search, screen/camera vision don't have an equivalent gap. Confirmed still-accurate, not regressed: `ConversationEngine.PersonalityRules.applyPersonality()` remains honestly-disclosed dead code; the movie-JARVIS personality prompt is real and wired on both the orchestrator and direct voice-interface paths.
    - Typecheck clean throughout. Full test suite re-run after all changes: 19 pass / 3 fail, the same pre-existing flakiness as every prior pass in this project - no new failures. **Not yet live-confirmed in a real `bun run dev listen` session** - the DB-level read/write path is real and verified; whether a real conversation actually surfaces a relevant memory naturally (not via a hand-written test query) needs Gavin's next real session.
...
- ✅ **Adaptive Processing Step 2 — Intent/Complexity Router (2026-09-04)** — `core/intent-router.ts`'s `classifyIntent()` is pattern-based (no LLM call to route). **Merge note:** originally wired as the first thing `VoiceInterface.generateResponse` does; reconciling with the live voice-pipeline work found `Orchestrator.processConversation()` already does the equivalent free/instant regex-based app-control detection (`parseAppControlIntent()`), so the router now runs on `generateResponse`'s no-orchestrator fallback path only, leaving one real dispatcher (not two) for the primary, orchestrator-backed path:
...
- ✅ **Adaptive Processing Step 4 — Gemini Live prototype (2026-09-04, CONFIRMED LIVE for real)** — new, deliberately isolated `src/prototypes/gemini-live/` (own directory, only reaches into the rest of the codebase for the one real tool it wires up — nothing here is imported by `VoiceInterface` or `Orchestrator`, matching the doc's "build and compare, don't integrate yet" instruction for this step). **Merge-session update:** Gavin's real `GEMINI_API_KEY` landed for the first time this session — the first-ever real connection found two real bugs invisible to typecheck/unit tests (neither opens a real socket): `setup.responseModalities` is rejected outright by the live API (belongs nested under `generationConfig`), and the transcribed default model (`gemini-2.0-flash-live-001`) is rejected as unsupported for `bidiGenerateContent` — `gemini-3.1-flash-live-preview` is the real one confirmed to connect, complete setup, and stream real audio. Fixing the model-rejection case also surfaced a third bug: a server-rejected setup closes the socket without ever firing an `error` event, so `connect()` used to hang forever instead of failing fast — now rejects with the real close code/reason. Verified twice: a raw isolated WebSocket test and the real `bun run dev live-prototype` CLI command, both showing genuine streamed 24kHz PCM audio starting ~250-900ms after the turn is sent — comfortably inside section 7's sub-2-second target. `src/tests/gemini-live-protocol.test.ts`/`gemini-live-session.test.ts` (7/7) unaffected.
...
- ⏸️ **Adaptive Processing Step 9 — Continue existing roadmap: correctly deferred, same reasoning as step 6.** The directive's own words: "Only after the realtime foundation is stable should additional advanced capabilities be prioritized." Step 6 (integrate) is still blocked on a real step 5 comparison — the realtime foundation isn't stable yet, it's an unverified prototype. Prioritizing further advanced capabilities ahead of that would be exactly the ordering the directive warns against. What step 9 asked for that *doesn't* depend on that ("fold each verified step's outcome into this ground-truth status block as it lands") has been done continuously through steps 1-8 above, not deferred.
  **Steps 1-3, 7, 8 are real and verified in this sandbox** (typecheck, full test suite, reasoning through actual code — 68 pass / 18 skip as of this line). **Steps 4-6 are honestly blocked on infrastructure this sandbox doesn't have**: a real `GEMINI_API_KEY`, a reachable Postgres (`bun run db:push` also still needs to run against one — the `action_journal` table from step 7 isn't live anywhere yet either), and at least one of Ollama/Gemini/OmniRoute reachable for the current-JARVIS side of the comparison. The concrete next action, in order: (1) run `bun run db:push` on Gavin's PC against a real Postgres, (2) `bun run dev compare-latency` with a real `GEMINI_API_KEY` and at least one model provider configured, (3) read the real numbers it prints, (4) only then decide step 6 — integrate or don't, based on what was actually measured, not what was hoped for.
- ✅ **Merge note (2026-09-04):** the Adaptive Processing branch above and this file's own extensive live-tested voice-pipeline history (Chatterbox, HUD, Spotify, screen/video/camera vision, proactive intelligence, persistent conversational memory) were built concurrently from the same commit and reconciled by hand — see this file's "Adaptive Processing update" note near the top for the real dispatch-precedence decision (`Orchestrator.processConversation()` stays primary; router/capability-registry/episode-cache back `VoiceInterface`'s no-orchestrator fallback path). `bun run db:push` still needs to run against a real Postgres before the `action_journal`/episode-cache tables are live anywhere.
