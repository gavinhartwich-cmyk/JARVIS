...
**Status:** ⚠️ Real core built and confirmed live 2026-09-02, not "Planned" anymore - but only a slice of Part 7.1's full monitoring list. `core/proactivity.ts` is the real relevance/urgency/permission/device-routing decision engine from Part 7.2's own diagram, reusing the already-real presence/authorization/memory infrastructure (Part 3) rather than reinventing it. Two real monitors feed it: `core/system-monitors.ts` (disk space, $0, no credentials) and `core/calendar-monitor.ts` (real Google Calendar, real OAuth refresh-token flow, credentials reused from Hartwich-OS - a separate project of Gavin's, same Google account, per his explicit live authorization). `bun run dev proactive-check` runs one real pass; verified three times in a row against Gavin's actual calendar - correct triage (an event 5 minutes out → notify immediately; hours-out events → archived) and confirmed real dedup (an unchanged condition doesn't re-notify). Email/Task/Business/Personal-health monitoring are real, disclosed, not-yet-built follow-up work - Gmail credentials exist in the same Hartwich-OS project but weren't reused this pass (a real open question: are those outreach-email accounts even the right thing for JARVIS to monitor, versus Gavin's own personal inbox - not decided blind). Also not built: a scheduled/looping wrapper that calls this periodically on its own - `proactive-check` currently only runs one pass on demand.

**Capabilities:**
- Event monitoring ✅ (real, for disk space + calendar; email/task/business/health not yet built - see status above)
