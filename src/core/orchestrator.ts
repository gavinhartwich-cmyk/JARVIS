...
import { spotifyPlayWithAutoOpen, spotifyPause, spotifyResume, spotifyNext, spotifyPrevious, spotifyStatus } from "./spotify";
  ...
  private async runSpotifyAction(
    ...
    try {
      ...
      switch (intent.action) {
        case "play":
          // [MOVED 2026-09-04] The real "no device -> open Spotify ->
          // retry" logic that used to live inline here is now
          // spotifyPlayWithAutoOpen() (core/spotify.ts) - a real, shared
          // function instead of logic only this path had, so Gemini Live
          // mode's play_music tool (live-tools.ts) gets the exact same
          // proactive behavior instead of a second copy of it.
          result = await spotifyPlayWithAutoOpen(intent.query!, async (target) => {
            try {
              const identity = await this.getIdentity();
              const screenControl = new ScreenControl();
              ...
              return { success: false, error: openError instanceof Error ? openError.message : String(openError) };
            }
          });
          break;
        case "pause":
      ...
      // [MOVED 2026-09-04] The real "no device -> open Spotify -> retry"
      // proactive behavior (per Gavin: "he needs to know if I ask him to
      // play a song to open Spotify and then play the song... he needs to
      // be proactive not reactive") now lives inside spotifyPlayWithAutoOpen()
      // itself (core/spotify.ts), called above - nothing further to do
      // here, `result` already reflects its outcome.
