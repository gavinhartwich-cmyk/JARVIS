...
export class IntelligentModelRouter implements IModelRouter {
  ...
  private getDefaultModels(): Map<string, LLMConfig> {
    ...
    // Fast models - quick response for simple tasks
    ...
    // Main model - balanced performance
  ...
  private applyPreferences(
    ...
    if (this.userPreferences.budget === "free") {
      adjusted.model = "claude-haiku-4-5"; // Cheapest option
    ...
    if (this.userPreferences.preferredProvider) {
      adjusted.provider = this.userPreferences.preferredProvider as
        | "claude"
        | "gemini"
        | "openai"
        | "local";
